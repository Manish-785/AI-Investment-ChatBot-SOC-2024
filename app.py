...
# Precompute sector embeddings once; the previous implementation recomputed
# nlp(sector) for every token in every query.
sector_docs = {sector: nlp(sector) for sector in sector_keywords}

def find_most_similar_sector(text):
    lowered = text.lower()
    # Explicit sector mentions are more reliable than token-level similarity.
    for sector in sorted(sector_keywords, key=len, reverse=True):
    ...
    for token in doc:
        ...
        for sector, sector_doc in sector_docs.items():
            similarity = sector_doc.similarity(token)
            if similarity > max_similarity:
                max_similarity = similarity
                most_similar_sector = sector
    return most_similar_sector if max_similarity > 0.6 else "others"
...
def get_top_companies(sector, top_n=10):
    ...
    return tickers.get(sector, [])[:top_n]
...
def fetch_data(ticker):
    return yfinance.download(ticker, start='2000-01-01')
...
def calculate_net_profit(data):
    ...
    return yearly_prices.pct_change().dropna() * 100

def plot_volatility(tickers, risk='medium'):
    risk_threshold = {'low': 0.2, 'medium': 0.35, 'high': 0.5}
    ticker_data = []
    plt.figure(figsize=(10, 6))
    for ticker in tickers:
        data = fetch_data(ticker)
...
# Goal phrases describe an investor's objective rather than a stock sector.
# This prevents sector names such as "healthcare" from being confused with goals.
...
# Precompute phrase embeddings once. This both improves consistency and removes
# repeated model.encode() calls from the per-query path.
goal_phrases = [(goal, phrase) for goal, phrases in investment_goals_mapping.items() for phrase in phrases]
goal_embeddings = model.encode([phrase for _, phrase in goal_phrases], convert_to_tensor=True, normalize_embeddings=True)

def find_most_similar_investment_goal(text):
    lowered = text.lower()
    # Exact phrase matching handles unambiguous user intents deterministically.
    exact_matches = []
    for goal, phrase in goal_phrases:
        if re.search(rf"\b{re.escape(phrase.lower())}\b", lowered):
            exact_matches.append(goal)
    if exact_matches:
        return exact_matches[0]

    text_embedding = model.encode(text, convert_to_tensor=True, normalize_embeddings=True)
    similarities = util.cos_sim(text_embedding, goal_embeddings)[0]
    best_index = int(similarities.argmax().item())
    best_score = float(similarities[best_index].item())
    return goal_phrases[best_index][0] if best_score > 0.5 else "others"
...
def extract_entities(text):
    ...
    # Explicit risk phrases are resolved deterministically; no-risk queries
    # correctly default to medium rather than inventing a user preference.
    explicit_risk = re.findall(r"\b(low|medium|high)\b", text.lower())
    if explicit_risk:
        if 'high' in explicit_risk:
            entities['RISK'] = 'high'
        elif 'medium' in explicit_risk:
...
def fundamental_analysis(ticker):
    ...
    info = stock.info
    return {'Ticker': ticker, 'P/E Ratio': info.get('trailingPE', 'N/A'), 'Earnings Growth': info.get('earningsGrowth', 'N/A'), 'Dividend Yield': info.get('dividendYield', 'N/A'), 'Debt-to-Equity': info.get('debtToEquity', 'N/A'), 'ROE': info.get('returnOnEquity', 'N/A')}
...
def technical_analysis(ticker):
    ...
    return {'Ticker': ticker, 'MA50': ma50, 'MA200': ma200, 'RSI': rsi, 'MACD': macd}
...
def evaluate_stocks(tickers):
    ...
    for fundamental, technical in zip(fundamental_results, technical_results):
        score = 0
        if fundamental['P/E Ratio'] != 'N/A' and fundamental['P/E Ratio'] < 20: score += 1
        if fundamental['Earnings Growth'] != 'N/A' and fundamental['Earnings Growth'] > 0.1: score += 1
        if fundamental['Debt-to-Equity'] != 'N/A' and fundamental['Debt-to-Equity'] < 1: score += 1
        if fundamental['ROE'] != 'N/A' and fundamental['ROE'] > 0.15: score += 1
        if technical.get('MACD', 0) > 0: score += 1
...
def predict_stock_prices(ticker, time=5, amount=100000):
    data = fetch_data(ticker)['Close']
    np.random.seed(42)
    model_forecast = ExponentialSmoothing(data, trend='add', seasonal='add', seasonal_periods=12)
    fit = model_forecast.fit()
    ...
    if not forecast_with_noise.empty:
        ...
        net_profit = 0; expected_returns = 0
...
def generate_summary(fundamental, technical):
    ...
    input_text = (f"The stock {fundamental['Ticker']} has the following fundamental metrics: a P/E Ratio of {fundamental['P/E Ratio']}, an Earnings Growth of {fundamental['Earnings Growth']}, a Dividend Yield of {fundamental['Dividend Yield']}, a Debt-to-Equity ratio of {fundamental['Debt-to-Equity']}, and a Return on Equity (ROE) of {fundamental['ROE']}. In terms of technical indicators, it has a 50-Day Moving Average of {technical['MA50']}, a 200-Day Moving Average of {technical['MA200']}, an RSI of {technical['RSI']}, and a MACD of {technical['MACD']}.")
...
def convert_date(date_str):
    years_match = re.compile(r'(\d+)\s*years?').match(date_str)
    if years_match: return float(years_match.group(1))
    months_match = re.compile(r'(\d+)\s*months?').match(date_str)
    if months_match: return float(months_match.group(1)) / 12
    days_match = re.compile(r'(\d+)\s*days?').match(date_str)
    if days_match: return float(days_match.group(1)) / 365
    return 5.0
...
if prompt:
    ...
    with st.chat_message('assistant'):
        ...
        time = convert_date(response.get('DATE', '5 years'))
        amount = response.get('AMOUNT', ['100000'])[0]
        def extract_amount(amount_str):
            match = re.search(r'\d+', amount_str)
            return int(match.group(0)) if match else 100000
